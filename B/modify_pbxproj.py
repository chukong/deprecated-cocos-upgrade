...
class XcodeProject(PBXDict):
    ...
    def remove_file_by_path2(self, file_path):
        objs = self.data.get('objects')
        fileRefID = ""
        buildPhase = "PBXResourcesBuildPhase"
        for key in objs:
            obj = objs.get(key)
            if obj.get('isa') == "PBXReferenceProxy" and obj.get("path") == file_path:
                fileRefID = key
                buildPhase = FILE_TYPE_INFO.get(obj['fileType'], "PBXResourcesBuildPhase")
                objs.remove(key)
                break
